...
export function renderLeaderboard(container: HTMLElement): void {
  ...
  title.style.cssText = 'font-size:16px;font-weight:700;color:#1F2937;margin-bottom:4px';
  ...
  colHeader.style.cssText = [
    ...
    'padding:6px 12px;font-size:10px;font-weight:700;color:#9CA3AF',
  ...
  getLeaderboard().then(entries => entries).catch(() => []).then(entries => entries.forEach((entry: any, i: number) => {
    ...
    row.style.cssText = [
      ...
      `border:1px solid ${isCurrent ? '#C4B5FD' : '#F3F4F6'}`,
